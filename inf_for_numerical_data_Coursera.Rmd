...
<li> The births </li> YYYYYYYYYYYYYY
</ol>
...
<li> Both distributions are extremely right skewed. </li> YYYYYYYYYYYYY
...
<li> We are 95% confident that babies born to nonsmoker mothers are on average 0.05 to 0.58 pounds heavier at birth than babies born to smoker mothers. </li> YYYYYYYYYYYYYYY
</ol>
```{r habit-weight-ci}
...
<li>(6.9779 , 7.2241)</li>
<li>(38.0952 , 38.5742)</li> YYYYYYYYYYYYYYYY
</ol>
```{r weeks-ci-99}
...
```{r gained-mature-ht}
# type your code for the Exercise here, and Knit
inference(x = mature, y = gained, data = nc, statistic = "mean", type = "ht", method = "theoretical", alternative = "twosided", null = 0)
```
...
#nc %>%
 # group_by(mature) %>%
#  summarise(range(mage)) XXXXXXXXXX
...
inference(x = premie, y = mage, data = nc, statistic = "mean", type = "ht", method = "theoretical", alternative = "twosided", null = 0)
```
