...
<ol>
<li> Observational study </li> YYYYYYYYYY
<li> Experiment </li>
</ol>
...
<li> Yes, revise wording to "Is there an association between beauty and course evaluations?" </li> YYYYYYYYYY
...
<li> The left skewness of the data suggests that the students are less likely to rate the professors highly. </li> YYYYYYYYYY
</ol>
```{r score-dist}
...
<li> Nearly normal residuals: Residuals are right skewed, but the sample size is large, so this may not be an important violation of conditions. </li> YYYYYYYYYYY
<li> Independent observations: Classes sampled randomly, no order effect. </li>
</ol>
```{r diag-score-bty_avg}
# type your code for the Question 5 here, and Knit

# linear relation b/w numerical x and y
...
<li> Tenure Track, Tenured, Teaching </li> YYYYYYYYY
...
</div> cls_profs | number of professors teaching sections in course in sample: single, multiple.
...
<li> 0.12 points higher than minority professors, all else held constant. </li> YYYYYYYYYY
...
```{r p-val-select}
# type your code for the Exercise here, and Knit
m_notfull <- lm(score ~ rank + ethnicity + gender + language + age + cls_perc_eval 
...
m6 <- lm(score ~ rank + ethnicity + gender + language + age + cls_students + cls_level + cls_profs + cls_credits + bty_avg 
             + pic_outfit + pic_color, data = evals)
...
```{r}
# remove pic_outfit
m12 <- lm(score ~ rank + ethnicity + gender + language + age + cls_perc_eval 
             + cls_students + cls_level + cls_profs + cls_credits + bty_avg 
...
```{r}
# remove pic_color
m13 <- lm(score ~ rank + ethnicity + gender + language + age + cls_perc_eval 
             + cls_students + cls_level + cls_profs + cls_credits + bty_avg 
...
```{r}
# full minus cls_profs
m_notfull <- lm(score ~ rank + ethnicity + gender + language + age + cls_perc_eval 
             + cls_students + cls_level + cls_credits + bty_avg 
             + pic_outfit + pic_color, data = evals)
summary(m_notfull)$adj.r.squared
```
```{r}
# remove rank
m_notfull1 <- lm(score ~ ethnicity + gender + language + age + cls_perc_eval 
             + cls_students + cls_level + cls_credits + bty_avg 
             + pic_outfit + pic_color, data = evals)
summary(m_notfull1)$adj.r.squared
```


```{r}
# remove ethnicity
m_notfull2 <- lm(score ~ rank + gender + language + age + cls_perc_eval 
             + cls_students + cls_level + cls_credits + bty_avg 
             + pic_outfit + pic_color, data = evals)
summary(m_notfull2)$adj.r.squared
```


```{r}
# remove gender
m_notfull3 <- lm(score ~ rank + ethnicity + language + age + cls_perc_eval 
             + cls_students + cls_level + cls_credits + bty_avg 
             + pic_outfit + pic_color, data = evals)
summary(m_notfull3)$adj.r.squared
```


```{r}
# remove language
m_notfull4 <- lm(score ~ rank + ethnicity + gender + age + cls_perc_eval 
             + cls_students + cls_level + cls_credits + bty_avg 
             + pic_outfit + pic_color, data = evals)
summary(m_notfull4)$adj.r.squared
```


```{r}
# remove age
m_notfull5 <- lm(score ~ rank + ethnicity + gender + language + cls_perc_eval 
             + cls_students + cls_level + cls_credits + bty_avg 
             + pic_outfit + pic_color, data = evals)
summary(m_notfull5)$adj.r.squared
```


```{r}
# remove cls_perc_eval
m_notfull6 <- lm(score ~ rank + ethnicity + gender + language + age
             + cls_students + cls_level + cls_credits + bty_avg 
             + pic_outfit + pic_color, data = evals)
summary(m_notfull6)$adj.r.squared
```


```{r}
# remove cls_students
m_notfull7 <- lm(score ~ rank + ethnicity + gender + language + age + cls_perc_eval 
             + cls_level + cls_credits + bty_avg 
             + pic_outfit + pic_color, data = evals)
summary(m_notfull7)$adj.r.squared
```


```{r}
# remove cls_level
m_notfull8 <- lm(score ~ rank + ethnicity + gender + language + age + cls_perc_eval 
             + cls_students + cls_credits + bty_avg 
             + pic_outfit + pic_color, data = evals)
summary(m_notfull8)$adj.r.squared
```


```{r}
# remove cls_credits
m_notfull9 <- lm(score ~ rank + ethnicity + gender + language + age + cls_perc_eval 
             + cls_students + cls_level + bty_avg 
             + pic_outfit + pic_color, data = evals)
summary(m_notfull9)$adj.r.squared
```


```{r}
# remove bty_avg
m_notfull10 <- lm(score ~ rank + ethnicity + gender + language + age + cls_perc_eval 
             + cls_students + cls_level + cls_credits 
             + pic_outfit + pic_color, data = evals)
summary(m_notfull10)$adj.r.squared
```


```{r}
# remove pic_outfit
m_notfull11 <- lm(score ~ rank + ethnicity + gender + language + age + cls_perc_eval 
             + cls_students + cls_level + cls_credits + bty_avg 
             + pic_color, data = evals)
summary(m_notfull11)$adj.r.squared
```


```{r}
# remove pic_color
m_notfull12 <- lm(score ~ rank + ethnicity + gender + language + age + cls_perc_eval 
             + cls_students + cls_level + cls_credits + bty_avg 
             + pic_outfit, data = evals)
summary(m_notfull12)$adj.r.squared
```

```{r}
# full minus cls_profs >>> final model
m_notfull <- lm(score ~ rank + ethnicity + gender + language + age + cls_perc_eval 
             + cls_students + cls_level + cls_credits + bty_avg 
             + pic_outfit + pic_color, data = evals)
summary(m_notfull)
...
Teaching, non minority, male, english language, young, with higher pct of students evaluating, higher number of students, upper class, one credit, attractive, black&white pic, formal
